/* CODIGO EMISOR V40 - FIX PIN VEXT 36 */
...
// --- CONFIGURACIÓN PINES HELTEC V3 (CORREGIDOS) ---
#define SDA_OLED 17
#define SCL_OLED 18  
#define RST_OLED 21
#define Vext 36      // <--- ¡CORREGIDO! Antes estaba mal (25)
...
// resto config...
const char* ssid_ap_default = "HP_Emisor_V40";     
...
String getHtml() {
  ...
  if(configMode) h += "<h2 style='color:orange'>MODO CONFIG</h2>"; else h += "<h2>EMISOR V40</h2>";
...
void setup() {
    Serial.begin(115200);
    pinMode(LED_PIN, OUTPUT); digitalWrite(LED_PIN, HIGH); // LED ENCENDIDO AL INICIO

    // --- FIX PANTALLA (Power Cycle Correcto) ---
    pinMode(Vext, OUTPUT);
    digitalWrite(Vext, LOW); delay(500); // LOW = ENCENDIDO en Heltec V3
    
    pinMode(RST_OLED, OUTPUT);
    digitalWrite(RST_OLED, LOW); delay(100);
    digitalWrite(RST_OLED, HIGH); delay(100);

    screen.init();
    screen.flipScreenVertically();
    screen.setFont(ArialMT_Plain_10);
    screen.clear(); screen.drawString(0,0,"Iniciando V40..."); screen.display();
    // -------------------------------------------
    ...
    digitalWrite(LED_PIN, LOW); // LED APAGADO (Listo)
}
...
void reconnect() {
    String id = "E40-"+String(random(0xffff),HEX);
